...
make -C bao-baremetal-guest PLATFORM=crosscon_soc ARCH_SUB=$ARCH_SUB MEM_BASE=0x60000 riscv_march=$PLATFORM_RISCV_ISA riscv_mabi=$PLATFORM_RISCV_ABI
...
make -C bao-baremetal-guest2 PLATFORM=crosscon_soc ARCH_SUB=$ARCH_SUB MEM_BASE=0x70000 riscv_march=$PLATFORM_RISCV_ISA riscv_mabi=$PLATFORM_RISCV_ABI
...
export CONFIG=spike-spmp-dual-multivcpu
...
make -C opensbi V=1 DEBUG=1 PLATFORM=${OPENSBI_PLATFORM} FW_PAYLOAD_PATH=$(realpath bao-hypervisor/bin/$BAO_PLATFORM/$CONFIG/bao.bin) -j12 PLATFORM_RISCV_ISA=$PLATFORM_RISCV_ISA PLATFORM_RISCV_ABI=$PLATFORM_RISCV_ABI 
